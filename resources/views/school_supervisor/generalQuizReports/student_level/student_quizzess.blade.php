...
@section('buttons')
    <a href="{{ route("school-branch-supervisor.general-quizzes-reports.student.level.student.quizzes.charts", array_merge(["student" => $user->id], request()->all())) }}" class="btn btn-md btn-success align-right" style="margin: 5px;">{{ trans("general_quizzes.view chart") }}</a>
    @if(count($studentQuizzes))
        <a href="{{ route('school-branch-supervisor.general-quizzes-reports.student.level.student.quizzes.export', array_merge(["student" => $user->id], request()->all())) }}" class="btn btn-md btn-success align-right" style="margin: 5px;" target="_blank">{{ trans("app.Export") }}</a>
    @endif
@endsection
                                ...
                                <tr class="text-center">
                                    ...
                                    <td>
                                        <a class="btn btn-xs btn-info"
                                           href="{{ route('school-branch-supervisor.general-quizzes-reports.student.level.student.section.performance', ["generalQuizStudent" => $studentQuiz->id]) }}"
