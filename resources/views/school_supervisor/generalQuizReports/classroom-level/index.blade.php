...
@section('buttons')
    <a href="{{ route("school-branch-supervisor.general-quizzes-reports.branch.reports.class.levels.charts", array_merge(["branch" => $branch ?? null], request()->all())) }}" class="btn btn-md btn-success align-right">{{ trans("general_quizzes.view chart") }}</a>
    @if(count($generalQuizzes))
        <a href="{{ route("school-branch-supervisor.general-quizzes-reports.student.reports.class.levels.export", request()->query->all()) }}" target="_blank" class="btn btn-md btn-success align-right" style="margin: 10px;">{{ trans("app.Export") }}</a>
    @endif
@endsection
...
@section('content')
    <div class="row">
        ...
        @if (!empty($generalQuizzes))
            @include('school_supervisor.generalQuizReports.classroom-level._filter')
