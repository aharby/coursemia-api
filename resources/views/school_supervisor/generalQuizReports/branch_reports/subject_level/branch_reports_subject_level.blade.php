...
@section('buttons')
    <a href="{{ route("school-branch-supervisor.general-quizzes-reports.branch.reports.subject.levels.charts", array_merge(["branch" => $branch ?? null], request()->all())) }}" class="btn btn-md btn-success align-right" style="margin: 5px;">{{ trans("general_quizzes.view chart") }}</a>
    @if (!empty($generalQuizzes))
        <a href="{{ route("school-branch-supervisor.general-quizzes-reports.branch.reports.subject.levels.export", array_merge(["branch" => $branch ?? null], request()->all())) }}" class="btn btn-md btn-success align-right" style="margin: 5px;" target="_blank">{{ trans("app.Export") }}</a>
    @endif
@endsection
...
@section('content')
    <div class="row">
        ...
        @if (!empty($generalQuizzes))
            @include('school_supervisor.generalQuizReports.branch_reports.subject_level._filter')
