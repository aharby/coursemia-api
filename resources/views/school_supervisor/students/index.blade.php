...
@section('content')
    <div class="row">
        ...
        @section('buttons')
            @if(!request()->has("deactivated"))
                <div class="col-md-2 col-sm-2 col-xs-2" data-step="2" data-intro="Export Students" data-position='right'>
                    <a href="{{ route('school-branch-supervisor.students.get.export', ["branch" => $branch ?? null]).'?'.request()->getQueryString() }}"
                       class="btn btn-primary">{{ trans('students.Export') }}</a>
                </div>
            @endif
        @endsection
        @include('school_supervisor.students._filter',['username'=>true,'mobile' => false,'classroom' => true])
                                    ...
                                    @if($row->user)
                                        ...
                                        <td>
                                            @if(can('view-students'))
                                                <a class="btn btn-primary btn-xs" href="{{ route('school-branch-supervisor.students.get.view-student', $row->id) }}"
                                            ...
                                            @if(!request()->has("deactivated"))
                                                <a class="btn btn-xs btn-info"
                                                   href="{{ route('school-branch-supervisor.students.get.edit', ["student" => $row->id, "branch" => $branch ?? null]) }}"
        ...
        {!! $rows->appends(Request::capture()->except('page'))->render() !!}
        </div>
        @else
