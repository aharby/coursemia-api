...
@section('buttons')
    <a href="{{ route("school-admin.general-quizzes-reports.branch-reports.question.percentage.report.index.charts", array_merge(["branch" => $branch ?? null], request()->all())) }}" class="btn btn-md btn-success align-right">{{ trans("general_quizzes.view chart") }}</a>
@endsection
...
@section('content')
    <div class="row">
        ...
        @if(!empty($generalQuizzes))
            @include('school_admin.GeneralQuizzesReports.branch_reports.question_percentage._filter')
                            ...
                            <table class="table">
                                ...
                                @foreach($generalQuizzes as $quiz)
                                    <tr class="text-center">
                                        ...
                                        <td>{{ round($quiz->studentsAnswered->average('score_percentage'),2) }} %</td>
                                        <td>
                                            <a class="btn btn-primary btn-xs" href="{{ route('school-admin.general-quizzes-reports.branch-reports.question.percentage.report.questions', $quiz->id) }}"
