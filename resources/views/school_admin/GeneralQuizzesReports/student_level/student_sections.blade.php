...
@section('buttons')
    <a href="{{ route("school-admin.general-quizzes-reports.student.level.student.section.performance.charts", ["generalQuizStudent" => $generalQuizStudent->id]) }}" class="btn btn-md btn-success align-right" style="margin: 5px;">{{ trans("general_quizzes.view chart") }}</a>

    @if(count($studentSectionsGrade))
        <a href="{{ route("school-admin.general-quizzes-reports.student.level.student.section.performance.export", ["generalQuizStudent" => $generalQuizStudent->id]) }}" class="btn btn-md btn-success align-right" style="margin: 5px;" target="_blank">{{ trans("app.Export") }}</a>
    @endif
@endsection
