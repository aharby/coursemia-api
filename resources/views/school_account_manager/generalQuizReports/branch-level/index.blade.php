...
@section('buttons')
    @if(count($generalQuizzes))
        <a href="{{ route("school-account-manager.general-quizzes-reports.student.reports.branch.levels.export", request()->query->all()) }}" target="_blank" class="btn btn-md btn-success align-right" style="margin: 10px;">{{ trans("app.Export") }}</a>
    @endif
@endsection
...
@section('content')
    <div class="row">
        ...
        @if(!empty($generalQuizzes))
            @include('school_account_manager.generalQuizReports.branch-level._filter')
                            ...
                            <table class="table">
                                ...
                                @foreach($generalQuizzes as $quiz)
                                    <tr class="text-center">
                                        ...
                                        <td>
                                            <a class="btn btn-primary btn-xs" href="{{ route('school-account-manager.general-quizzes-reports.branch-level-report.students', $quiz->id) }}"
...
@push('scripts')
    <script>
        ...
        $("#gradeClass_id").change(function () {
            ...
            $.get('{{ route('school-account-manager.manager-reports.gradeClass.subjects') }}' + `/${gradeClass_id}/${branch_id}`,
                {
                    '_token': $('meta[name=csrf-token]').attr('content'),
                    'gradeClass': $(this).val(),
                })
