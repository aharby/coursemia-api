...
@section('buttons')
    <a href="{{ route("school-account-manager.general-quizzes-reports.sectionPercentageReport.charts", $generalQuiz->id) }}" class="btn btn-md btn-success align-right" style="margin: 5px;">{{ trans("general_quizzes.view chart") }}</a>
    @if(count($sections))
        <a href="{{ route("school-account-manager.general-quizzes-reports.sectionPercentageReport.export", array_merge(request()->query->all(), ["generalQuiz" => $generalQuiz->id])) }}" class="btn btn-md btn-success align-right" style="margin: 5px;" target="_blank">{{ trans("app.Export") }}</a>
    @endif
@endsection
