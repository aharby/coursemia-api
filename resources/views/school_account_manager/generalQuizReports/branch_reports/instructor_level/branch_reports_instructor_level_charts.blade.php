...
@push('scripts')
    <script>
        ...
        $("#instructor_id").change(function () {
            ...
            $.get('{{ route('school-branch-supervisor.classrooms.getInstructorSubjects') }}' + `/${instructor_id}?classroom=`+$('#classroom_id').val()+`&gradeClass=`+$('#gradeClass_id').val(),
                {
                    '_token': $('meta[name=csrf-token]').attr('content'),
                    'instructor': $(this).val(),
                })
                ...
                datasets: [
                    {
                        ...
                        data: [
                            ...
                            "{{ $quiz->successful_percentage ?? 0.00 }}"
                        ]
                    }
                ]
            },
