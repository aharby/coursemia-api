...
@section('buttons')
    ...
    <a href="{{ route("school-account-manager.general-quizzes-reports.branch.reports.instructor.levels.charts", array_merge(["branch" => $branch ?? null], request()->all())) }}" class="btn btn-md btn-success align-right" style="margin: 10px;">{{ trans("general_quizzes.view chart") }}</a>
    @if(count($generalQuizzes))
        <a href="{{ route("school-account-manager.general-quizzes-reports.branch.reports.instructor.levels.export", request()->query->all()) }}" target="_blank" class="btn btn-md btn-success align-right" style="margin: 10px;">{{ trans("app.Export") }}</a>
    @endif
@endsection
...
@section('content')
    <div class="row">
        ...
        @if (!empty($generalQuizzes))
            @include('school_account_manager.generalQuizReports.branch_reports.instructor_level._filter')
