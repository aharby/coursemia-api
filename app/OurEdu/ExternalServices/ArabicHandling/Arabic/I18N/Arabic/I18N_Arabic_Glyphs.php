...
namespace App\OurEdu\ExternalServices\ArabicHandling\Arabic\I18N\Arabic;
 ...
 *
 * Filename:   I18N_Arabic_Glyphs.php
 *
...
class I18N_Arabic_Glyphs
{
    ...
    protected function preConvert($str)
    {
        ...
        for ($i = $max - 1; $i >= 0; $i--) {
            ...
            if (($i - 2) >= 0 and $prevChar && mb_strpos($this->_vowel, $prevChar) !== false) {
                $prevChar = $chars[$i - 2];
                if (($i - 3) >= 0 and $prevChar && mb_strpos($this->_vowel, $prevChar) !== false) {
                    $prevChar = $chars[$i - 3];
                }
            }
            ...
            if (($prevChar == 'لا' || $prevChar == 'لآ' || $prevChar == 'لأ'
                ...
                if (($i - 2) >= 0 and mb_strpos($this->_prevLink, $chars[$i - 2]) !== false) {
                    $form++;
                }

                if (($i - 1) >= 0 and mb_strpos($this->_vowel, $chars[$i - 1])) {
                    $output .= '&#x';
