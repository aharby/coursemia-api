...
class ExpensesTransactionTransformer extends TransformerAbstract
{
    ...
    public function transform(PaymentTransaction $transaction)
    {
        ...
        return [
            ...
            'payment_method' => $transaction->payment_method == PaymentEnums::VISA ? trans('payments.visa') : trans(
                'payments.wallet'
            ),
           'description' => trans('app.transaction_desc',
                   [
    ...
    public function includeUser($transaction)
    {
        if ($transaction->receiver) {
            return $this->item($transaction->receiver, new UserTransformer(), ResourceTypesEnums::USER);
        }
    }
}
