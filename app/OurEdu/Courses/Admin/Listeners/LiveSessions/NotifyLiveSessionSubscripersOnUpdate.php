...
class NotifyLiveSessionSubscripersOnUpdate
{
    /**
     * Handle the event.
     *
     ...
     * @return void
     */
    public function handle(LiveSessionUpdated $event)
    {
...
/*
        ...
        (new MailManger)->prepareMail([
            ...
            'subject' => trans('emails.Live Session updated'),
            'emails' => $subscripedUsers->toArray(),
            'view' => 'liveSessionUpdated'
        ])->handle();
**/
    }
    ...
    private function sendGroupMail(string $local, array $subscripers, $event)
    {
        if (count($subscripers) > 0) {
            ...
            (new MailManger)->prepareMail([
                ...
                'data' => ['url' => UrlActionEnums::getUpdatedLiveSessionUrl($event->liveSession),
               'lang' => $local],
                'subject' => trans('emails.Live Session updated',[], $local),
                'emails' =>  $subscripers,
                'view' => 'liveSessionUpdated'
            ])->handle();
         }
