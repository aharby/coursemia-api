    ...
    public function includeActions(LiveSession $liveSession)
    {
        ...
        if ($authUser = Auth::guard('api')->user()) {
            ...
            if ($authUser->type == UserEnums::STUDENT_TYPE && $student = $authUser->student) {
                ...
                if (! $userIsSubscriped) {
                    if (isset($this->params['auto_join']) && $isSessionRunning) {
                        $actions[] = [
                            'endpoint_url' => buildScopeRoute('api.student.liveSessions.subscribeAndJoin', ['liveSessionId' => $liveSession->id]),
                            'label' =>trans('vcr.Subscribe and Join'),
                            'method' => 'POST',
                            'key' => APIActionsEnums::LIVE_SESSION_SUBSCRIBE_AND_JOIN
                        ];
                    }else {
                        $actions[] = [
            ...
            }elseif ($authUser->type == UserEnums::INSTRUCTOR_TYPE && $student = $authUser->instructor && $liveSession->instructor_id == $authUser->id && $isSessionRunning){
                //student is subscribed to session, then return join action
                $actions[] = [
                    'endpoint_url' => buildScopeRoute('api.instructor.online-sessions.join-session', ['sessionId' => $liveSession->id]),
                    'label' => trans('app.Join Session'),
                    'method' => 'POST',
