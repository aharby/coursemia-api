...
class QuizApiController extends BaseApiController
{
    ...
    public function __construct(
        ...
        $this->middleware('type:student')
            ->only(['startQuiz', 'getNextOrBackQuestion', 'finishQuiz', 'postAnswer']);
    ...
    {
        $useCase = $this->startQuizUseCase->startQuiz($quizId);
        try {
                if ($useCase['status'] == 200) {
                    ...
                    return $this->transformDataModInclude(
                        $questions,
                        '',
                        new QuizQuestionTransformer($params),
    ...
    public function getNextOrBackQuestion(int $quizId, Request $request)
    {
        $page = request()->input('page') ?? 1;
        try {
            $nextOrBackQuestion = $this->nextBackUseCase->nextOrBackQuestion($quizId, $page);
    ...
    public function finishQuiz(int $quizId)
    {
        try {
            $useCase = $this->finishQuizUseCase->finishQuiz($quizId);
            if ($useCase['status'] == 200) {
                $params['result'] = $useCase['result'];
                $meta = ['message' => trans('quiz.Quiz is over')];

                return $this->transformDataModInclude($useCase['quiz'],
                    '',
                    new QuizTransformer($params),
                    ResourceTypesEnums::QUIZ, $meta);
            } else {
                return formatErrorValidation($useCase);
            }
        } catch (\Throwable $e) {
    ...
    public function postAnswer(BaseApiRequest $request, $quizId)
    {
        ...
        try {
            ...
            $this->answerQuizQuestionUseCase->postAnswer($quizId, $data);
            DB::commit();
