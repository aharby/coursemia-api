...
class PeriodicTestStudentListTransformer extends TransformerAbstract
{
    ...
    public function transform(StudentQuiz $studentQuiz)
    {
        $transformedData = [
            ...
            'quiz_time' => (string) $studentQuiz->quiz->quiz_time,
        ];
    ...
    public function includeActions(StudentQuiz $studentQuiz)
    {
        $actions = [];
        $actions[] = [
            'endpoint_url' => buildScopeRoute('api.periodic-test.get.student.periodic-test', [
                'PeriodicTestId' => $studentQuiz->quiz_id,
                'studentId' => $studentQuiz->student_id
            ]),
            'label' => trans('quiz.view'),
            'method' => 'GET',
            'key' => APIActionsEnums::VIEW_STUDENT_PERIODIC_TEST
        ];
        if (count($actions)) {
