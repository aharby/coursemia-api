...
class CompetitionQuestionTransformer extends TransformerAbstract
{
        ...
        if (isset($this->params['actions'])) {
            ...
            if (isset($this->params['finish_competition'])) {
                $actions[] = [
                    ...
                    'endpoint_url' => buildScopeRoute('api.student.courseCompetitions.post.finishCompetition', ['exam' => $question->exam_id]),
    ...
    public function includeQuestionData(ExamQuestion $question)
    {
        ...
        if($question->competitionQuestionStudents()
            ->where('student_id',auth()->user()->student->id)->first()){
            $this->params["is_answer"]  = true;
        }
    ...
    {
        return $this->item($question->exam, new CourseCompetitionTransformer($this->params), ResourceTypesEnums::COMPETITION);
    }
}
