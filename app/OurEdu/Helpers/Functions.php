...
use App\OurEdu\ExternalServices\ArabicHandling\Arabic\I18N\Arabic\I18N_Arabic_Numbers;
...
if (!function_exists('moveGarbageMedia')) {
    ...
    )
    {
        ...
        $garbageMedias = \App\OurEdu\GarbageMedia\GarbageMedia::find($ids);
        ...
        \App\OurEdu\GarbageMedia\GarbageMedia::whereIn('id', $ids)->delete();
    }
}
...
if (!function_exists('moveSingleGarbageMedia')) {
    ...
    {
        $garbageMedia = \App\OurEdu\GarbageMedia\GarbageMedia::find($id);
        $fileName = null;
        if ($garbageMedia) {
            if ($storagePath) {
                $fileName = $storagePath . '/' . $garbageMedia->filename;
...
if (!function_exists('calculateQuestionResult')) {
    ...
    function calculateQuestionResult(int $allCorrectAnswer, int $allAnswer)
    {
        if ($allAnswer == 0) {
            ...
            return \App\OurEdu\LearningResources\Enums\DifficultlyLevelEnums::percentageDifficultyLevel($percentageResult);
        }
    }
}
