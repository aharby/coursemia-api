...
class CourseHomeWorkTransformer extends TransformerAbstract
{
    ...
    public function transform($homework)
    {
        $transformerDatat = [
            ...
            'status' => ($this->getHomeworkStatus($homework) == GeneralQuizStatusEnum::STARTED) ? trans('general_quizzes.Started') : (($this->getHomeworkStatus($homework) == GeneralQuizStatusEnum::FINISHED)  ? trans('general_quizzes.Finished') :  trans('general_quizzes.Not_Started')),
        ];
    ...
    public function includeActions($homework)
    {
        ...
        if ($user = Auth::guard('api')->user()) {
            if ($this->getHomeworkStatus($homework) != GeneralQuizStatusEnum::FINISHED) {
                //Start Homework
                $actions[] = [
