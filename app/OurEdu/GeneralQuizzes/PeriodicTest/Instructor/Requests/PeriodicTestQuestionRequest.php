...
class PeriodicTestQuestionRequest extends BaseApiParserRequest
{
    ...
    public function rules()
    {
        ...
        return [
            'attributes' => 'required',
            'attributes.grade' => 'required|numeric|gte:0',
            ...
            'relationships.general_quiz_questions_data.data.questions.*' => [
                function ($attribute, $question, $fail) {
                    ...
                    if ($data->question_slug == QuestionsTypesEnums::DRAG_DROP_TEXT ||
                        ...
                        $this->validateQuestion('question', $question, $fail, $key);
                        $this->validateQuestion('question_feedback', $question, $fail, $key);
                        // $this->validateQuestionGrade('grade', $question, $fail, $key);
                    }
                }
            ],
    ...
    {
        if (!property_exists($question, $attribute) || $question->$attribute == '' || ctype_space($question->$attribute)) {
            return $fail(trans('general_quizzes.required', ['num' => $key + 1, 'field' => trans('general_quizzes.' . $attribute)]));
        }
    }
    ...
    public function messages()
    {
        return [
            ...
            'attributes.grade.gte' => trans('general_quizzes.grade_gte_zero')
        ];
    }
