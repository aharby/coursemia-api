...
class HwStudentTransformer extends TransformerAbstract
{
     ...
     */
    public function __construct(GeneralQuiz $generalQuiz = null, User $student = null, public $courseHomework = false)
    {
    ...
    public function transform(GeneralQuizStudent $student)
    {
        ...
        if ($student->user) {
            $data['is_active'] = (bool)$student->is_active;
            $data['name'] = trim($student->user->name);
        }
        return $data;
    }
        ...
        if ($generalQuizStudent->generalQuiz->questions->count()) {
            ...
            return $this->collection(
                ...
                new QuestionTransformer(
                    $this->generalQuiz,
                    $this->student,
                    ["show_if_is_correct" => true, 'course_homework' => $this->courseHomework]
                ),
    ...
    public function includeClassroom(GeneralQuizStudent $student)
    {
        if ($student->user->student->classroom) {
            return $this->item($student->user->student->classroom, new ClassroomLookUptransformer(), ResourceTypesEnums::CLASSROOM);
        }
    }
}
