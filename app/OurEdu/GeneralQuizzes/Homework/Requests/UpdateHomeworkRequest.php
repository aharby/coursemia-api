...
//        dd($this->sections);
        ...
        if ($this->route('homeworkId')) {
            $homework = GeneralQuiz::whereId($this->route('homeworkId'))->firstOrFail();
    ...
    protected function prepareForValidation()
    {
        if ($user = Auth::guard('api')->user()) {
            ...
            $this->sections = SubjectFormatSubject::
                where('subject_id', $this->validationData()['attributes']['subject_id'] ?? -1)
                ->pluck('id')->toArray();
        }
    }
}
